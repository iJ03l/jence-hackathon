...
export default function CreatorDashboardPage() {
    ...
    const [isPostModalOpen, setIsPostModalOpen] = useState(() => searchParams.get('new') === 'true')
    const [activePostMenu, setActivePostMenu] = useState<string | null>(null)
    const [postToDelete, setPostToDelete] = useState<string | null>(null)
    const [isDeleting, setIsDeleting] = useState(false)

    // Clear search param when modal opens
    useEffect(() => {
        if (isPostModalOpen && searchParams.has('new')) {
            const newParams = new URLSearchParams(searchParams)
            newParams.delete('new')
            newParams.delete('vertical')
            setSearchParams(newParams, { replace: true })
        }
    }, [isPostModalOpen, searchParams, setSearchParams])
                                            ...
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {post.verticalName && (
                                                    <span className="text-xs text-jence-gold flex items-center gap-0.5">
                                                        <FileText size={10} /> {post.verticalName}
                                                    </span>
                                                )}
                                                {post.isPinned && (
            ...
            {isPostModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-background border border-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 relative animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <CreatePostForm
                            creatorId={creatorId}
                            initialVerticalId={searchParams.get('vertical') || verticalId}
...
function CreatePostForm({ creatorId, initialVerticalId, onClose, onSuccess }: any) {
    ...
    useEffect(() => {
        api.getVerticals().then((data) => {
            setVerticals(data)
            if (!selectedVerticalId && data.length > 0) {
    ...
    const handleSubmit = async () => {
        ...
        if (!creatorId || selectedVerticalId === '') {
            alert('Please select a section before publishing.')
            return
        }
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="none">My Profile Only (No Section)</option>
                        {verticals.map((v) => (
                                ...
                                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                    {uploadingImage ? (
                                        ...
                                        <>
                                            ...
                                            <p className="text-xs text-muted-foreground">Jence supports JPG, PNG, GIF, WebP, AVIF, or HEIC up to 5MB</p>
                                        </>
                                    )}
                                </div>
                            </label>
                ...
                <div className="flex items-center justify-between gap-4 rounded-xl border border-border bg-muted/10 p-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                            Let readers send one-time tips to this article.
                        </p>
                    </div>
